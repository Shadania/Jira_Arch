...
from script_shared import count_set_overlaps_and_return_sets, get_issue_is_parent
...
def plot_precision_data(tag = ""):
    ...
    ax.legend()
    fig.set_size_inches(8,4.5)
    plt.savefig(f'figures/maven_{tag}_precision.png')
    if show_figures:
        plt.show()
...
def make_maven_issues_json(AK):
    ...
    non = 'non-' if not AK else ''
    with open(f"analysis-output/maven-{non}AK.json", 'w+') as outfile:
        json.dump({'issues': output_issues}, outfile)

def make_bhat_issues_json(AK):
    issues = []
    first = True
    with open('data/bhat_labels.csv') as f:
        for line in f:
            if first:
                first = False
                continue
    ...
    non = 'non-' if not AK else ''
    with open(f"analysis-output/bhat-{non}AK.json", 'w+') as outfile:
        json.dump({'issues': issues}, outfile)

def find_parents():
    # check if we've done this before
    parents_path = f'parents/parents.json'
...
make_maven_issues_json(False)
make_maven_issues_json(True)
make_bhat_issues_json(True)
make_bhat_issues_json(False)
